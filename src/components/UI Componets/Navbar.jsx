...
import { Navlinks, ShowNavItems, SideNavLinks } from '../UI Componets/index'
...
const Navbar = () => {
    ...
    return (
        ...
        <div className='navbar fixed z-50 w-full overflow-x-hidden'>
            <div className='px-4 py-2  font-ceraRegular bg-black'>
                <div className='w-full min-h-[3.5rem] flex flex-row  items-center justify-between 
                    lg:gap-20 lg:px-14
                '>
                    <button onClick={() => setNavlinks(!navlinks)} 
                        ...
                        <>
                            {
                                navlinks ? 
                                <RxCross2 fontSize="1.8rem" color='white' /> :
                                <RxHamburgerMenu fontSize="1.8rem" color='white' />
                            }
                        </>
                    </button>
                    <div className='logo lg:w-fit flex flex-row items-center lg:justify-normal'>
                        <NavLink to='/'>
                            <svg width="132" height="30" viewBox="0 0 132 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <Navlinks navlinks={navlinks} setNavlinks={setNavlinks} categoryHandler={categoryHandler} />
                    ...
                    <div className='flex flex-row gap-2 items-center '>
                        ...
                        <NavLink to='cart'>
                            <button className='relative flex items-center'>
                                <MdOutlineShoppingCart style={{ color: 'white', fontSize: '1.7rem' }} />
                                {
                                    // cart.length &&
                ...
                <div className={`${showSearch ? 'block' : 'hidden'}`}>
                    <div className='flex flex-row gap-2 items-center h-10 px-4 lg:px-14 py-6 transition-all duration-500'>
                        <HiOutlineSearch style={{ color: 'white', fontSize: '1.5rem' }} />
                        <input className='w-full bg-transparent text-white px-4 py-1 outline-none placeholder:text-white'
                            type="text"
                            placeholder='Search'
                        />
                        <RxCross2 onClick={() => setShowSearch(!showSearch)}
                            style={{ color: 'white', fontSize: '1.6rem' }} 
                        />
                    </div>
                    
                    {/* <div className='absolute top-full -px-4 z-50 w-full h-[100vh] bg-[#0000007c] ' /> */}
                </div>
            </div>
            <SideNavLinks navlinks={navlinks} setNavlinks={setNavlinks} />

            {/* on hover show categories */}
            <div>
                {
                    hover &&
