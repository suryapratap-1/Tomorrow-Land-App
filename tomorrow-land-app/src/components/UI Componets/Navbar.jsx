...
import { ShowNavItems } from '../UI Componets/index'
...
const Navbar = () => {
  ...
  const [category, setCategory] = useState(navOptions[0].tag)
  ...
  return (
    ...
    <div className='navbar fixed z-50 w-full overflow-x-hidden transition-all duration-500'>
      <div className={`px-4 md:px-16 py-2 bg-black font-ceraRegular`}>
        <div className=' flex flex-row gap-16 items-center h-14'>
          <div>
            <NavLink to='/'>
          ...
          <div className='flex flex-row gap-5 items-center text-sm  text-white'>

            <div onMouseEnter={categoryHandler}
              className='relative'>
              <NavLink to='men' className='flex flex-col items-center'>
                <h3>MEN</h3>
                {/* <AiFillCaretDown className={`${showIcon ? 'opacity-100' : 'opacity-0'}`} /> */}
              </NavLink>
            </div>

            <div onMouseEnter={categoryHandler}>
              <NavLink to='women' className='flex flex-col items-center' >
                <h3>WOMEN</h3>
                {/* <AiFillCaretDown className={`${showIcon ? 'opacity-100' : 'opacity-0'}`} /> */}
              </NavLink>
            </div>
            ...
            <div onMouseEnter={categoryHandler}>
              <NavLink to='kids' className='flex flex-col items-center'>
                <h3>KIDS</h3>
                {/* <AiFillCaretDown className={`${showIcon ? 'opacity-100' : 'opacity-0'}`} /> */}
              </NavLink>
            </div>
            ...
            <div onMouseEnter={categoryHandler}>
              <NavLink to='accessories' className='flex flex-col items-center'>
                <h3>ACCESSORIES</h3>
                {/* <AiFillCaretDown className={`${showIcon ? 'opacity-100' : 'opacity-0'}`} /> */}
              </NavLink>
            </div>
            ...
            <div onMouseEnter={categoryHandler}>
              <NavLink to='art' className='flex flex-col items-center'>
                <h3>ART</h3>
                {/* <AiFillCaretDown className={`${showIcon ? 'opacity-100 rotate-180' : 'opacity-0 -rotate-180'} transition-all duration-300`} /> */}
              </NavLink>
            </div>
          ...
          <div className='flex flex-row gap-3 items-center'>
            <button onClick={() => setShowSearch(!showSearch)}>
              <HiOutlineSearch style={{ color: 'white', fontSize: '1.7rem' }} />
            </button>
            <button>
              <FiHeart style={{ color: 'white', fontSize: '1.7rem' }} />
            </button>
            <NavLink to='cart'>
              <button onClick={() => setShowCart(!showCart)} className='flex items-center'>
                <HiOutlineShoppingCart style={{ color: 'white', fontSize: '1.7rem' }} />
              </button>
            </NavLink>
          </div>
        ...
        {/* searchbar */}
        <div className={`flex flex-row gap-2 items-center h-10 py-6 duration-200
                ${showSearch ? 'block' : 'hidden'} `}
        >
          <HiOutlineSearch style={{ color: 'white', fontSize: '1.5rem' }} />
          <input
            className='w-full bg-transparent text-white px-4 py-1 outline-none placeholder:text-white'
            type="text"
            placeholder='Search'
          />
          <RxCross2 style={{ color: 'white', fontSize: '1.6rem' }} />
        </div>
...

      {
        ...
        <div onMouseLeave={() => {
          ...
          className='w-full px-20 py-12  bg-white'>
          {
            <ShowNavItems items={navOptions.filter((options) => options.tag == category)} />
          }
        </div>
      }
